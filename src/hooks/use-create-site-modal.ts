...
type UseCreateSiteModalParams = {
  open: boolean
  onOpenChange: (open: boolean) => void
  onCreated?: () => void
}

export function useCreateSiteModal({ open, onOpenChange, onCreated }: UseCreateSiteModalParams) {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        setMessage({ type: "error", text: response.result.error || "Error al agregar el sitio" })
        return
      }
