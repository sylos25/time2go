...
-- ─────────────────────────────────────────────────────────────────────────────
-- fn_evento_eliminar
-- Elimina un evento y todas sus relaciones en cascada ordenada dentro de una
-- sola transacción. Usa bloqueo pesimista para garantizar integirad atómica.
-- Respuesta: { ok: true,  id_evento }
...
BEGIN
  ...
  -- ── 2. Contexto de auditoría ──────────────────────────────────────────────
  PERFORM set_config('app.id_usuario', p_id_usuario_editor::TEXT, TRUE);

  -- ── 3. Bloqueo pesimista + verificación de existencia ────────────────────
  PERFORM 1
  FROM  tabla_eventos
  ...
  -- ── 4. Eliminación en cascada (relaciones antes que el padre) ────────────
